package com.shopping.monitor.monitorservice.heatbeat;


import com.shopping.monitor.monitorservice.runner.HeartPluginConfig;
...
public class HeartSenderHttp {
    ...
    public String sendHttpPost() {
        ...
        try {
            ...
            logger.info(HeartPluginConfig.getInstance().getIpPort() + "：[心跳发送成功！]" + new Date());
         } catch (Exception e) {
            e.printStackTrace();
            logger.info(HeartPluginConfig.getInstance().getIpPort() + "：[心跳消息发送失败！]"+ new Date());
        } finally {
            //关闭资源
